...
class GreetingScanner:
    ...
    def approve_one(self) -> Optional[dict]:
        ...
        try:
            ...
            random_delay(self.settings)
            time.sleep(random.uniform(1.0, 2.0))  # 等待跳转到对话框

            # 尝试从 topbar 获取昵称
            name = None
            try:
                topbar_elem = self.elements.get("chat_topbar_friend", {})
                title_cfg = topbar_elem.get("topbar_title", {})
                title_rid = title_cfg.get("resourceId", "")
                if title_rid:
                    try:
                        tel = self.driver.d(resourceId=title_rid)
                        if tel.exists:
                            name = (tel.get_text() or "").strip()
                    except Exception:
                        self._logger.debug("获取 topbar 昵称失败", exc_info=True)
            except Exception:
                self._logger.debug("解析 topbar 配置失败", exc_info=True)
