...
1. **Root**：Magisk 授权 `com.momoqun.agent`，或 LDPlayer 等模拟器在设置里开启 ROOT（`adb shell su -c id` 应显示 `uid=0`）。
2. **输入法**（中文 `type_text` 需要）：打开 **MomoQun Agent** → **打开输入法设置** → 启用并切换默认为「MomoQun IME」。
...
> 真实模拟器上 `dump_hierarchy` 每次会 spawn 一次 `app_process` dumper（约 100–300ms），
> 但深层 UI 树不再因 `uiautomator dump` 栈溢出而崩溃。
...
| `dump_hierarchy` 失败 | 仅本地 root：`su -c pm path com.momoqun.agent` → `su -c CLASSPATH=<base.apk> app_process … com.momoqun.agent.dumper.Main`，XML 经 WS 回 master（**不**走 `shell_exec`/adb）。排查：`adb shell su -c id`；`logcat -s MQAgent.Dump:* MQAgent.Shell:*`；Magisk 是否对 `com.momoqun.agent` 始终允许；master `allow_shell_exec` 可为 false |
